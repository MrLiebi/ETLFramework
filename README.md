...
PowerShell-based **ETL framework** (release **1.0.0**) for repeatable data pipelines: interactive **project wizard**, modular **sources** and **destinations**, central **logging** and **validation**, optional **adapters** (e.g. Flexera), and integration with the **Windows Credential Manager**.
...
On disk, this is the same tree as after `git clone` or extracting a release ZIP (e.g. `PS-ETL-Framework-v1.0.0`).
...
Current release line in the repository: **1.0.0** (see `CHANGELOG.md` and comments in entry scripts).
...
2. Create an **annotated tag** for that commit, e.g. `git tag -a v1.0.0 -m "PS ETL Framework 1.0.0"` then `git push origin v1.0.0`.
3. On GitHub, open **[Releases](https://github.com/MrLiebi/ETLFramework/releases)** → **Draft a new release** → choose that tag → set the title (e.g. `1.0.0`) → paste the matching **CHANGELOG** section into the description → **Publish release**.
