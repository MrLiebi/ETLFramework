...
- Added a bundled offline installer at `Templates/Installers/DotNet/NDP481-x86-x64-AllOS-ENU.exe` so the wizard can install .NET Framework 4.8.1 without requiring a separate download step.
...
### Changed
- Bumped the framework pre-final release line to **0.23.0** across entry scripts, templates, runtime modules, wizard banner, and documentation.
...
### Changed
- Raised the framework pre-final release line to **0.22.0** across templates, runtime, wizard banner, and entry scripts.
...
### Changed
- Kept the framework pre-final release line aligned across templates, runtime, wizard, and tests.
...
### Changed
- Finalized the framework as the consolidated pre-final release `0.20.0`.
...
- Bumped the entire framework, templates, setup scripts, and release metadata to `0.19.0`.
